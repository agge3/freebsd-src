...
-- xxx manual override build system for testing
local debug = true
...
-- Returns tokens normalized into an expected form. Examples of normalization
-- are removing unwanted characters and splitting pointers off their operands
-- into two tokens.
...
-- xxx no reason to have this with checkType(), but keeping for now to keep
-- driver code working. The problem was that there's no set "order" for how
-- types are defined in `syscalls.master`.
local function typeToStr(sc)
	...
	-- (mostly) reverse alphabetical. xxx wrong
	for i = #types, 1, -1 do
		if i == 1 then
...
-- correctly. This function generates the tokens from what we initially parsed.
...
-- This function compares our makesyscalls tokens (after processing) to
-- reparsing `syscalls.master` line-by-line.
local function cmpScmasterNative(tbl)
	local fh = assert(io.open(sysfile, "r"))
	if not fh then	-- xxx needs to match rest of library
		print("ERROR: could not open " .. sysfile)
	end
	local scmaster = genScmasterNative(tbl)

	local idx = 1		-- `syscalls.master` index
	local buf_idx = 1	-- makesyscalls index
